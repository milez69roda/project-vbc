...
class Reports_Model extends CI_Model { 
    ...
    public function terms($startdate, $enddate, $type){ 
		...
		$this->db->select("club_transaction_terms.id,pay_ref, CONCAT(ai_fname,' ', ai_lname) AS full_name,
...
/* 	public function expired(){
	...
	public function expired_35_days(){
		...
		$this->db->where(" (pay_status ='3' AND term_type NOT IN(".TERM_EXPIRED.", ".TERM_SUSPENSION.", ".TERM_TERMINATION.", ".TERM_DELETED.", ".TERM_ROLLING_MONTLY.") ) ", null, false);
		$this->db->where(" (exp_date - INTERVAL 35 DAY) < CURRENT_DATE ", null, false);
		$this->db->join('club_membership', 'club_membership.mem_id = club_transaction.mem_id', 'LEFT OUTER');
		$this->db->order_by('club_transaction.exp_date', 'desc');
		$results = $this->db->get('club_transaction')->result();
		return $results;
	}
